...
class Graph:
    def __init__(self, adjacency_list:dict[str, set[str]]):
        ...
        self.__edges = set()
        for key_word, neighbors in adjacency_list.items():
            for neighbor in neighbors:
                self.__edges.add(tuple(sorted([key_word, neighbor])))
        ### END SOLUTION
    ...
    def is_valid_path(self, path:tuple[str]) -> bool:
        ...
        if path[0].lower() not in self.__adjacency_list:
            return False

        for i in range(1, len(path)):
            if path[i].lower() not in self.__adjacency_list[path[i - 1].lower()]:
                return False
            
        return True
        ### END SOLUTION

    def get_shortest_bfs_path(self, start:str, target:str, other_tuples = set()) -> tuple[str]:
    ...
    def get_shortest_bfs_path_length(self, start:str, target:str) -> int:
        ...
        '''
        ### BEGIN SOLUTION
        path = self.get_shortest_bfs_path(start, target)
    ...
    def get_all_shortest_bfs_paths(self, start:str, target:str) -> set[tuple[str]]:
        ...
        '''
        ### BEGIN SOLUTION
        init_path = self.get_shortest_bfs_path(start, target)
        path_lists = set()
        ...
        while len(init_path) == length:
            ...
            init_path = self.get_shortest_bfs_path(start, target, path_lists)
...
#   print(f"Shortest Path Length from {start} to {target}:", myGraph.get_shortest_bfs_path_length(start, target))
  ...
  print(myGraph2.get_shortest_bfs_path("fool", "sage"))
  print(myGraph2.get_shortest_bfs_path_length("fool", "sage"))
