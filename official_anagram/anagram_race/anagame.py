...
def generate_letters(fun_factor: int, distribution: str, explorer:AnagramExplorer) -> list:
    ...
    if distribution == "uniform":
        anagrams = set()
        while len(anagrams) < fun_factor:
            for i in range(7):
                letters[i] = chr(random.randint(97, 122))
            anagrams = explorer.get_all_anagrams(letters)
        return letters
    elif distribution == "scrabble":
        anagrams = set()
        while len(anagrams) < fun_factor:
            letter_distribution = ['E', 'E', 'E', 'E', 'E', 'E', 'E', 'E', 'E', 'E', 'E', 'E','A', 'A', 'A', 'A', 'A', 'A', 'A', 'A', 'A','I', 'I', 'I', 'I', 'I', 'I', 'I', 'I', 'I','O', 'O', 'O', 'O', 'O', 'O', 'O', 'O','N', 'N', 'N', 'N', 'N', 'N','R', 'R', 'R', 'R', 'R', 'R','T', 'T', 'T', 'T', 'T', 'T','L', 'L', 'L', 'L','S', 'S', 'S', 'S','U', 'U', 'U', 'U','D', 'D', 'D', 'D','G', 'G', 'G','B', 'B','C', 'C','M', 'M','P', 'P','F', 'F','H', 'H','V','V','W','W','Y', 'Y','K','J','X','Q','Z']
            for i in range(7):
                letters[i] = random.choice(letter_distribution).lower()
                letter_distribution.remove(letters[i].upper())
            anagrams = explorer.get_all_anagrams(letters)
    ### END SOLUTION 
    return letters
...
def calc_stats(guesses: list, letters: list, explorer) -> dict:
    ...
    for guess in guesses:
        if explorer.is_valid_anagram_pair(guess, letters) and tuple(sorted(list(guess))) not in sorted_guesses:
            stats["valid"].append(guess)
            sorted_guesses.append(tuple(sorted(list(guess))))
            stats["score"] += len(guess[0]) - 2
        else:
            stats["invalid"].append(guess)
    
    try:
        stats["accuracy"] = int(((len(stats["valid"])/len(guesses)) * 100) // 1)
    except ZeroDivisionError:
        stats["accuracy"] = int(0)
    
    stats["guessed"] = {value for tuple in stats["valid"] for value in tuple}
    stats["not guessed"] = all_anagrams.difference(stats["guessed"])
    stats["skill"] = int((100 * len(stats["guessed"]) / len(all_anagrams)) // 1)
    ### END SOLUTION 
    return stats
