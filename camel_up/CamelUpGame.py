...
class CamelUpGame:
    ...
    def get_player_move(self, player: CamelUpPlayer)->str:
        ...
        while choice.lower() not in ["b", "r", "p"]:
    ...
    def get_ticket_EV(self, ticket_value:int, prob_first:float, prob_second:float)->float:
        ...
        ev = 0
        ### BEGIN SOLUTION
        prob_losing = 1 - prob_first - prob_second
        ev = ticket_value * prob_first + prob_second - prob_losing
        ### END SOLUTION
        return ev
    ...
    def play_1_leg(self):
        ...
        while not self.board.is_leg_finished():
            ...
            match move:
                ...
                case "p":
                    self.board.place_spectator(player)
            self.board.print(self.players)
            curr_player = (curr_player + 1) % 2
        
        for player in camelup.players:
            print(f"{player.name} ended the leg with {player.money} coins.")
            player.bets = [] #print("decoc")

        self.board.pyramid = set(self.board.camel_colors)
        self.board.ticket_tents = {color:self.board.BETTING_TICKET_VALUES.copy() for color in self.board.camel_colors}
        self.board.dice_tents = []
        
        return self.board.is_leg_finished()
